...
- **摘要替代中段**：早期的搜索、计算结果已经完成历史使命，模型要的是"做过什么、关键数据是什么"，不需要逐字原文
...
`main.ts` 的任务要求研究四个主题并逐条统计字符数，步数足够多，能把历史推过阈值（演示配置 `maxTokens: 400`，故意调小让压缩必触发）：
...
最后一问是**压缩质量检验**：等模型回答时，早期的搜索结果和字符数已经被压缩掉了，它只能从摘要里回忆。"哪个主题字符数最多、具体多少"这种精确问题，摘要漏一个数字就答不出来。
...
- **把摘要提示词里的第 2 条删掉再跑**：观察摘要丢失精确数字后，最终答案怎么错，体会压缩提示词就是压缩质量的全部杠杆
