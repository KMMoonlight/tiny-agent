...
- **改提示词要动 Agent 源码**。提示词本质是"配置"，却长在"实现"里——调一句措辞就要改核心循环的文件
...
- **第 4 步仍然停下来等审批**。提示词里没有写任何"保存笔记需要批准"的规则，也拦不住什么——`save_note` 是 `sensitive` 级，第四章的权限钩子照样介入。提示词管"模型想做什么"，代码管"模型能做什么"
- **第 5 步的最终答案长这样**：
...
格式不是模型"碰巧"给的，是提示词里的 Output Format 一节规定的。改那一节，格式就变——**不动 Agent 一行代码**。
...
循环逻辑本身没有任何变化。这就是"依赖注入"在一个 150 行 Agent 里的样子：**谁组装 Agent，谁决定提示词**——`main.ts` 可以给客服 Agent 和运维 Agent 装同一套循环、不同的提示词。
...
同一个任务、同一份 Agent 代码，**只动提示词配置就改变了模型的行为**——这就是把提示词当依赖注入的意义。

## 提示词不是安全边界

本章必须同时讲清楚一个反面观点：**写在提示词里的规则，只是对模型的"请求"，不是"强制"**。
